...
def printer_600_plan(checkout, cfg):
    ...
    * ``600.cfg`` goes in the *stock* printer's directory, because the shared
    ...
    ``maintenance.cfg`` rides along in the same directory. Its service macros
    are not included by anything -- the operator adds one ``[include]`` line to
    printer.cfg -- but shipping them here means they follow configurator updates
    instead of living as a copy-paste block that nothing tracks.
